...
def reddata():
    ...
    cursor.execute('select asin,best_rank,price,score,sold_by,seller_num,reviews from stores where spider_time = "20180604"')

    data1 = cursor.fetchall()

    cursor.execute('select asin,best_rank,price,score,sold_by,seller_num,reviews from stores where spider_time = "20180605"')
...
def save_to_file(data1,data2):
    ...
    with open('0605.txt', 'w', encoding='utf-8') as f:
        f.write('Holy Stone店铺商品排名增长信息 -- 0605'+'\n')
        f.write('\n')
        ...
        for item in data1:
            ...
            if rank.isdigit() and int(rank) < 5000:
                ...
                # f.write('asin码:{0}  price:{1}  评分:{2}'.format(item['asin'], item['price'], item['score']) + '\n')
                # f.write('卖家: {0} 其他卖家数量：{1}'.format(item['sold_by'], item['seller_num']) + '\n')
                # f.write('reviews增长数量：{0}'.format(int(item['reviews']), item['best_rank'].strip()[1:])+'\n')
                # f.write('现在排名：{0} 昨天排名：{1} 增长排名'.format(item['reviews'], item['best_rank'].strip()[1:])+'\n')
                # f.write('\n')
        for item in data2:
            # print(item)
            rank = item['best_rank'].strip()[1:].replace('#', '')
